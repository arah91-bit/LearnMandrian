...
def test_beginner_arc_never_runs_out_of_material():
    ...
    threshold of stage k+1. Asserted through leaving S4 (Phase 3 closed it
    with hand-checked vocabulary) — move the
...
def test_s5_vocabulary_milestone_only_rises():
    ...
    for k, s in enumerate(curriculum.STAGES):
        ...
        vocab |= {w[0] for t in reader.TEXTS
                  if reader._LEVEL_IDX[t["level"]] <= k for w in t["new_words"]}
        if s["id"] == "S5":
            assert len(vocab) >= 1500, f"S5 milestone: {len(vocab)}/1500"
            break
